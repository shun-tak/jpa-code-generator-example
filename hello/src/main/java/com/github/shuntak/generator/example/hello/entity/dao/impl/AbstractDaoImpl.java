...
/** Abstract DAO class. Auto generated by JPA code generator. */
...
public abstract class AbstractDaoImpl<E> implements AbstractDao<E> {
  ...
  protected int update(CriteriaUpdate<E> criteriaUpdate) {
    int rows =
        getEntityManager().createQuery(Objects.requireNonNull(criteriaUpdate)).executeUpdate();
    getEntityManager().flush();
    return rows;
  }

  protected int delete(CriteriaDelete<E> criteriaDelete) {
    int rows =
        getEntityManager().createQuery(Objects.requireNonNull(criteriaDelete)).executeUpdate();
    getEntityManager().flush();
    return rows;
  }
}
